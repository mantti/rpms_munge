...
Source3:        tmp-files-munge.conf
Patch0:			munge-tmp-files.patch
...
#cp -p %{SOURCE3} tmp-files-munge.conf
...
install -p -D -m 644 tmp-files-munge.conf %{buildroot}/%{_sysconfdir}/tmpfiles.d/munge.conf
...
%config(noreplace) %{_sysconfdir}/tmpfiles.d/munge.conf
...
%changelog
* Wed Jun 20 2018 Mika Anttila <mika.anttila@tut.fi> - 0.5.11-4
- Changed /var/run/munge to be created by systemd

* Mon Aug 15 2016 Steve Traylen <steve.traylen@cern.ch> - 0.5.11-3
- License is LGPLv3+ for the libs.
